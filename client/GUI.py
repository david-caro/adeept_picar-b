...
def getposBgr(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        tcpClicSock.send(
            (
                "FCSET %s %s %s" % (HSVimg[y, x][0], HSVimg[y, x][1], HSVimg[y, x][2])
            ).encode()
        )
...
def servo_buttons(x, y):
    ...
    Btn_3 = tk.Button(
        root, width=8, text="SpeechR", fg=color_text, bg=color_btn, relief="ridge"
    )
    ...
    Btn_SR = tk.Button(
        root, width=8, text="CV Run", fg=color_text, bg=color_btn, relief="ridge"
    )
    ...
    Btn_Rainbow = tk.Button(
        root, width=8, text="Rainbow", fg=color_text, bg=color_btn, relief="ridge"
    )
...
def scale_FL(x, y, w):
    ...
    Btn_Render = tk.Button(
        root, width=10, text="Render", fg=color_text, bg="#212121", relief="ridge"
    )
    Btn_Render.place(x=x + w + 111, y=y + 20)
    Btn_Render.bind("<ButtonPress-1>", call_Render)

    Btn_CVFL = tk.Button(
        root, width=10, text="CV FL", fg=color_text, bg="#212121", relief="ridge"
    )
...
def ultrasonic_radar(x, y):
    x_range = 2
    can_scan = tk.Canvas(
        root, bg=color_can, height=250, width=320, highlightthickness=0
    )  # define a canvas
    can_scan.place(x=x, y=y)  # Place the canvas
    line = can_scan.create_line(
        0, 62, 320, 62, fill="darkgray"
    )  # Draw a line on canvas
    line = can_scan.create_line(
        0, 124, 320, 124, fill="darkgray"
    )  # Draw a line on canvas
    line = can_scan.create_line(
        0, 186, 320, 186, fill="darkgray"
    )  # Draw a line on canvas
    line = can_scan.create_line(
        160, 0, 160, 250, fill="darkgray"
    )  # Draw a line on canvas
    line = can_scan.create_line(
        80, 0, 80, 250, fill="darkgray"
    )  # Draw a line on canvas
    line = can_scan.create_line(
        240, 0, 240, 250, fill="darkgray"
    )  # Draw a line on canvas

    can_tex_11 = can_scan.create_text(
        (27, 178), text="%sm" % round((x_range / 4), 2), fill="#aeea00"
    )  # Create a text on canvas
    can_tex_12 = can_scan.create_text(
        (27, 116), text="%sm" % round((x_range / 2), 2), fill="#aeea00"
    )  # Create a text on canvas
    can_tex_13 = can_scan.create_text(
        (27, 54), text="%sm" % round((x_range * 0.75), 2), fill="#aeea00"
    )  # Create a text on canvas
...
def new_number2view(x, y, info):
    ...
    for i in range(0, total_number):  # Scale the result to the size as canvas
        ...
        if dis_info_get > 0:
            len_dis_1 = int(
                (dis_info_get / x_range) * 250
            )  # 600 is the height of canvas
            pos = int((i / total_number) * 320)  # 740 is the width of canvas
            pos_ra = int(
                ((i / total_number) * 140) + 20
            )  # Scale the direction range to (20-160)
            len_dis = int(
                len_dis_1 * (math.sin(math.radians(pos_ra)))
            ...
            y1_l = y1_l - abs(
                math.sin(math.radians(pos_ra)) * 130
            )  # Orientation of line

            line = can_scan_1.create_line(
                pos, y0_l, x1_l, y1_l, fill=color_line
            )  # Draw a line on canvas
            point_scan = can_scan_1.create_oval(
                (pos + 3), y0, (pos - 3), y1, fill=color_oval, outline=color_oval
...
def scale_FC(x, y, w):
    ...
    def call_SET(event):
        tcpClicSock.send(
            (
                "FCSET %s"
                % rgb2hsv(int(var_R.get()), int(var_G.get()), int(var_B.get()))
            ).encode()
        )
...
def function_buttons(x, y):
    ...
    Btn_function_1 = tk.Button(
        root, width=8, text="RadarScan", fg=color_text, bg=color_btn, relief="ridge"
    )
    ...
    Btn_function_4 = tk.Button(
        root, width=8, text="LineTrack", fg=color_text, bg=color_btn, relief="ridge"
    )
    Btn_function_5 = tk.Button(
        root, width=8, text="Automatic", fg=color_text, bg=color_btn, relief="ridge"
    )
    Btn_function_6 = tk.Button(
        root, width=8, text="SteadyCam", fg=color_text, bg=color_btn, relief="ridge"
    )
    Btn_function_7 = tk.Button(
        root, width=8, text="Instruction", fg=color_text, bg=color_btn, relief="ridge"
    )
