...
if appConnection:
    try:
        ...
        AppConntect_threading = threading.Thread(
            target=appserver.app_ctrl
        )  # Define a thread for app ctrl
...
if Blockly:
    try:
        blockly_threading = threading.Thread(
            target=start_blockly
        )  # Define a thread for Blockly
        blockly_threading.setDaemon(
            True
        )  #'True' means it is a front thread,it would close when the mainloop() closes
